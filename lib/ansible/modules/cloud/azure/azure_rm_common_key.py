...
try:
    ...
    from azure.mgmt.keyvault.version import VERSION as keyvault_client_version
...
AZURE_EXPECTED_VERSIONS = dict(
    ...
    keyvault_client_version="0.31.0"
)
...
class AzureRMModuleBase(object):
    ...
    def keyvault_client(self):
        ...
        if not self._keyvault_client:
            self.check_client_version('keyvault', keyvault_client_version, AZURE_EXPECTED_VERSIONS['keyvault_client_version'])
            self._keyvault_client = KeyVaultManagementClient(
                self.azure_credentials,
                self.subscription_id,
                base_url=self.base_url
            )
            self._register('Microsoft.KeyVault')
